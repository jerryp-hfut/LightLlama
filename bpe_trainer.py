...
def bpe_trainer(input_path: str, vocab_size: int, special_tokens: List[str]) -> Tuple[Dict[int, bytes], List[Tuple[bytes, bytes]]]:
    ...
    try:
        ...
        print(f"错误：文件 '{input_path}' 未找到。请检查路径是否正确。")
        return {}, []
    ...
    for token in special_tokens:
        ...
        vocab_id += 1
    alphabet = set()
    for chars in word_splits:
        alphabet.update(chars)
    
    for char in sorted(alphabet):
        if char not in vocab.values():
            vocab[vocab_id] = char
            vocab_id += 1
    ...
    for i in range(num_merges):
        ...
        if not pairs:
            print(f"没有更多可合并的字符对，提前结束。当前合并次数: {i}")
            break
        ...
        for chars, freq in word_splits.items():
            ...
            while j < len(chars):
                if j < len(chars) - 1 and (chars[j], chars[j + 1]) == best_pair:
                    new_chars.append(chars[j] + chars[j + 1])  # 合并为字节序列
                    j += 2
                else:
        ...
        if (i + 1) % 100 == 0:
            print(f"完成 {i + 1}/{num_merges} 次合并，当前词汇表大小: {len(vocab)}")
